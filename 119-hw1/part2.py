...
=== ANSWER Q2b BELOW ===
The large pipeline has the highest throughput, which is the opposite of what I expected.
=== END OF Q2b ANSWER ===
"""
...
def q4a():
    ...
    h.add_pipeline("copy 1", lambda: add_list(LIST_SINGLE_ITEM))
    h.add_pipeline("copy 2", lambda: add_list(LIST_SINGLE_ITEM))
    h.add_pipeline("copy 3", lambda: add_list(LIST_SINGLE_ITEM))
    # raise NotImplementedError
    # Generate a plot.
...
=== ANSWER Q4b BELOW ===
The latency varied very little, with about 0.0008 milliseconds difference between the copies. This is less than I expected.
=== END OF Q4b ANSWER ===
"""
...
def q5a():
    ...
    size = len(df)
    h = ThroughputHelper()
    h.add_pipeline("part 1 pipeline", size, part1.PART_1_PIPELINE)
    h.compare_throughput()
    # raise NotImplementedError
    return h.throughputs
def q5b():
    ...
    size = len(df)
    h = LatencyHelper()
    h.add_pipeline("part 1 pipeline", lambda: part1.PART_1_PIPELINE)
    h.compare_latency()
    # raise NotImplementedError
    return h.latencies
...
def population_pipeline(df):
    ...
    import pandas as pd
    df_grouped = df.groupby('Entity').agg({'Year': ['min', 'max'], 'Population (historical)': ['first', 'last']})
    df_grouped.columns = ['Year_min', 'Year_max', 'Population_first', 'Population_last']
    df_grouped['Year_diff'] = df_grouped['Year_max'] - df_grouped['Year_min']
    df_grouped = df_grouped[df_grouped['Year_diff'] > 0]
    df_grouped['Pop_increase_per_year'] = (df_grouped['Population_last'] - df_grouped['Population_first']) / df_grouped['Year_diff']
    stats = df_grouped['Pop_increase_per_year'].describe()
    result = [float(stats['min']), float(stats['50%']), float(stats['max']), float(stats['mean']), float(stats['std'])]
    # raise NotImplementedError
    return result
...
def load_input_large():
    ...
    # raise NotImplementedError
    return pd.read_csv("data/population-large.csv")
...
===== ANSWER Q10 BELOW =====
The difference between the two pipelines is very big, especially in terms of throughput. The 'fromvar' pipelines consistently show higher throughput compared to the 'baseline' pipelines, showing that loading data from an existing DataFrame is much faster than reading from a file. In terms of latency, the difference is not as big but is still noticeable.
===== END OF Q10 ANSWER =====
"""
...
def for_loop_pipeline(df):
    ...
    for index, row in df.iterrows():
        ...
        country_data[country].append(year)
    results = []
    for country, years in country_data.items():
        min_year = min(years)
        max_year = max(years)
        median_year = pd.Series(years).median()
        mean_year = pd.Series(years).mean()
        std_year = pd.Series(years).std()
        results.append((min_year, median_year, max_year, mean_year, std_year))
    summary = pd.DataFrame(results, columns=['min', 'median', 'max', 'mean', 'std'])
    # raise NotImplementedError
    return summary.describe().loc[['min', '50%', 'max', 'mean', 'std']].values.flatten().tolist()
...
def for_loop_large():
    df = load_input('data/population-large.csv')
    pop = for_loop_pipeline(df)
    # raise NotImplementedError
    return pop
...
===== ANSWER Q14a BELOW =====
The baseline pipelines are much faster in throughput than the for loop pipelines.

14b. Which pipeline is faster in terms of latency?

===== ANSWER Q14b BELOW =====
The for loop pipeline is a bit faster in latency than the baseline pipeline.
===== END OF Q14b ANSWER =====
...
===== ANSWER Q14c BELOW =====
The experiment shows that the operations in Pandas are more efficient for processing large datasets than using for loops. The throughput difference indicates that for large amounts of data processing, using Pandas' built-in functions is much more efficient for performance. However, for very small datasets or single-row operations, it is not so necessary.
===== END OF Q14c ANSWER =====
"""
...
===== ANSWER Q15 BELOW =====
The biggest impact on performance was the vectorized operations in Pandas compared to the for loop implementations.
===== END OF Q15 ANSWER =====
...
===== ANSWER Q16 BELOW =====
Throughput tends to increase with the size of the input dataset. This is probably because of the overhead of setting up the computation being weaned off over a larger number of items, leading to more efficient processing as the dataset size increases.
===== END OF Q16 ANSWER =====
...
===== ANSWER Q17 BELOW =====
Throughput and latency are often inversely related. As throughput increases, latency usually decreases, and vice versa. This is likely because higher throughput means that more requests are being processed in parallel, leading to faster response times for individual requests.
===== END OF Q17 ANSWER =====
"""
...
def extra_credit_a():
    # Compare the cost of random sampling vs. the cost of getting rows from the DataFrame manually
    import numpy as np
    import pandas as pd
    df = pd.DataFrame(np.random.randn(100000, 3), columns=list('ABC'))
    h = ThroughputHelper()
    h.add_pipeline("random_sampling", len(df), lambda: df.sample(n=1000))
    h.add_pipeline("manual_rows", len(df), lambda: df.iloc[np.random.choice(df.index, size=1000, replace=False)])
    h.compare_throughput()
...
def extra_credit_b():
    # Compare the cost of random sampling vs. the cost of getting rows from the DataFrame manually for latency
    import numpy as np
    import pandas as pd
    df = pd.DataFrame(np.random.randn(100000, 3), columns=list('ABC'))
    h = LatencyHelper()
    h.add_pipeline("random_sampling", lambda: df.sample(n=1))
    h.add_pipeline("manual_rows", lambda: df.iloc[np.random.choice(df.index, size=1, replace=False)])
    h.compare_latency()
