...
def q2(dfs):
    ...
    columns = [tuple(df.columns) for df in dfs]
...
def q3(dfs):
    ...
    names = [set(df['university']) for df in dfs]
    for name in names:
        if name != names[0]:
            return False
    # Return:
...
=== ANSWER Q3b BELOW ===
The checks failed because the datasets for different years contain different universities.
=== END OF Q3b ANSWER ===
"""
...
def q4(dfs):
    ...
    sample = dfs[2].sample(5)
    print(sample)
    names = sample.iloc[:, 1].tolist()
...
=== ANSWER Q4b BELOW ===
Strengths:
1. There is a wide range of universities from different regions.
2. The dataset includes multiple attributes that contribute to the overall ranking.

Weaknesses:
1. It is unclear how the overall score is calculated.
2. The dataset may not include all relevant factors that contribute to university rankings, such as student satisfaction or employment outcomes after graduation.
3. There may be inconsistencies in how data is reported across years.
=== END OF Q4b ANSWER ===
"""
...
=== ANSWER Q6a BELOW ===
Test test_q3 fails because the set of university names is not the same across all years.
=== END OF Q6a ANSWER ===
...
=== ANSWER Q6b BELOW ===
test_q3 fails because it is intended to fail. The code is correct.
=== END OF Q6b ANSWER ===
...
def q8a(dfs):
    # Enter Code here
    for df in dfs:
        df_top100 = df[df['rank'] <= 100]
        print(df_top100['region'].value_counts())
    df_2021 = dfs[2]
    count_usa = df_2021[df_2021['region'] == 'USA'].shape[0]
...
=== ANSWER Q8b BELOW ===
It appears that the bigger and more developed the country, the more universities it has in the top 100. This would be consistent throughout the years because countries would not decrease in development or size in a short span of time.
=== END OF Q8b ANSWER ===
"""
...
def q9(dfs):
    # Enter code here
    df_2021 = dfs[2]
    averages = df_2021[['academic reputation', 'employer reputation', 'faculty student', 'citations per faculty', 'overall score']].mean().tolist()
...
def q10_helper(dfs):
    # Enter code here
    df_2021 = dfs[2]
    avg_2021 = df_2021.groupby('region')[['academic reputation', 'employer reputation', 'faculty student', 'citations per faculty', 'overall score']].mean().reset_index()
    return avg_2021
...
def q11(avg_2021):
    ...
    first_row = sorted_avg_2021.iloc[0]
    # raise NotImplementedError
    return first_row
...
=== ANSWER Q12b BELOW ===
Singapore is at the top because it scores highly in all attributes that are in the table.
=== END OF Q12b ANSWER ===
"""
...
def q13a(avg_2021):
    ...
    avg_2021.plot(kind='box', subplots=True, layout=(2,4), figsize=(12,8))
...
=== ANSWER Q13b BELOW ===
There is one point in the overall score plot that is an outlier, which would be Singapore.
=== END OF Q13b ANSWER ===
"""
...
=== ANSWER Q14b BELOW ===
There is sort of a trend of overall score increasing with academic reputation, but it is not very strong.
=== END OF Q14b ANSWER ===
...
def q15_helper(dfs):
    ...
    top_10 = pd.DataFrame()
    for i, df in enumerate(dfs):
        df_top10 = df[df['rank'] <= 10][['university', 'overall score']]
        df_top10 = df_top10.rename(columns={'overall score': f'overall score {2019 + i}'})
        if top_10.empty:
            top_10 = df_top10
        else:
            top_10 = pd.merge(top_10, df_top10, on='university')
    return top_10
...
def q17a(top_10):
    # Enter code here
    top_10.set_index('university').T.plot(marker='o', figsize=(12,8))
    plt.xlabel('Year')
    plt.ylabel('Overall Score')
    ...
    plt.savefig("output/part1-17a.png", bbox_inches='tight')
    plt.close()
    # raise NotImplementedError
...
=== ANSWER Q17b BELOW ===
MIT has remained consistently at 100 over the three years. UCL started at the lowest and increased over the years. Harvard decreased over the years.
=== END OF Q17b ANSWER ===
"""
...
def q18(dfs):
    # Enter code here
    df_2021 = dfs[2]
    corr = df_2021[['academic reputation', 'employer reputation', 'faculty student', 'citations per faculty', 'overall score']].corr()
    ...
    plt.savefig("output/part1-18.png", bbox_inches='tight')
    plt.close()
...
=== ANSWER Q19 BELOW ===
Interestingly, citations per faculty is heavily correlated with overall score, more so than anything else.
=== END OF Q19 ANSWER ===
"""
...
def q20a(dfs):
    df_2021 = dfs[2]
    df_2021['overall score'] = df_2021['overall score'] + (df_2021['university'] == 'University of California, Berkeley (UCB)') * 10
    df_2021 = df_2021.sort_values(by='overall score', ascending=False)
    berkeley_score = df_2021[df_2021['university'] == 'University of California, Berkeley (UCB)']['overall score'].values[0]
    # raise NotImplementedError
...
def q20b(dfs):
    ...
    df_2021['overall score'] = df_2021['overall score'] + (df_2021['university'] == 'University of California, Berkeley (UCB)') * 10
    df_2021 = df_2021.sort_values(by='overall score', ascending=False)
    top_10 = df_2021.head(10)['university'].tolist()
    # raise NotImplementedError
...
def q21():
    ...
    df_2021_falsified = df_2021_falsified[NEW_COLUMNS]
    df_2021_falsified = df_2021_falsified.sort_values(by='overall score', ascending=False)
    top_10 = df_2021_falsified.head(10)['university'].tolist()
    # raise NotImplementedError
    return top_10
...
=== ANSWER Q22 BELOW ===
I think that manipulating the source data directly would be most effective and most difficult to detect because it would not be obvious that the data had been altered unless one had access to the original data and was specifically looking for discrepancies.
=== END OF Q22 ANSWER ===
"""
