...
=== ANSWER Q2a BELOW ===
I would need to use a script like setup() when setting up a new environment for a team member, or when uploading the project to a new server. This ensures that everything would be properly set up and ready to run without manual intervention, saving time and reducing the risk of errors.
=== END OF Q2a ANSWER ===

Do you see an alternative to using a script like setup()?

=== ANSWER Q2b BELOW ===
An alternative to using a script like setup() could be to use Github Actions or other tools to automate the setup and deployment process.
=== END OF Q2b ANSWER ===
...
def setup_for_new_machine():
    ...
    subprocess.run(["wget", "https://raw.githubusercontent.com/DavisPL-Teaching/119-hw1/refs/heads/main/data/test-input.txt", "-O", "data/test-input.txt"])
    subprocess.run(["wget", "https://raw.githubusercontent.com/DavisPL-Teaching/119-hw1/refs/heads/main/data/test-output.txt", "-O", "output/test-output.txt"])
    # Copy script files
    subprocess.run(["cp", "test-script.py", "scripts/test-script.py"])
    # raise NotImplementedError
...
=== ANSWER Q4 BELOW ===
Data scientists in larger-scale projects must spend a significant portion of their time writing and maintaining setup scripts like setup() and setup_for_new_machine(). This is because ensuring that the environment is correctly configured is very important for reproducibility and collaboration among team members.
=== END OF Q4 ANSWER ===
...
=== ANSWER Q5 BELOW ===
I would expect that the script would work on a friend's machine, as long as they have the same operating system and architecture as mine. However, there could be some issues like missing dependencies or differences in package versions that could lead to compatibility problems.
=== END OF Q5 ANSWER ===
...
def pipeline_shell():
    # Use shell commands to count the number of rows in the file
    result = os.popen("cat data/population.csv | tail -n +2 | wc -l").read().strip()
...
def q6():
    ...
    if pipeline_shell() == pipeline_pandas():
...
def q7():
    ...
    h.add_pipeline("shell", 10000, pipeline_shell)
    h.add_pipeline("pandas", 10000, pipeline_pandas)
...
=== ANSWER Q9 BELOW ===
Pandas is significantly faster than using shell for both throughput and latency. This is because Pandas is optimized for data manipulation and can handle large datasets more efficiently than shell commands. 
=== END OF Q9 ANSWER ===
"""
